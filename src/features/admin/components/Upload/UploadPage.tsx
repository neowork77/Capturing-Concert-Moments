    ...
    <div className="min-h-screen py-16 px-4 sm:px-6 lg:px-8 bg-[#FFFBFC] text-[#3D3040] flex flex-col justify-between">
      <div className="max-w-xl w-full mx-auto">
        ...
        <div className="mb-8">
          <Link
            href="/admin"
            className="inline-flex items-center gap-2 text-xs font-semibold uppercase tracking-wider text-[var(--ink-muted)] hover:text-[#F4A0B5] transition-all duration-300 group"
          >
        ...
        <div className="text-center mb-10">
          ...
          <p className="text-sm text-[var(--ink-muted)] max-w-sm mx-auto font-light leading-relaxed">
        ...
        <div className="bg-white rounded-3xl p-6 sm:p-8 border border-[rgba(0,0,0,0.04)] shadow-[0_8px_30px_rgb(0,0,0,0.01)] backdrop-blur-sm">
          ...
          {files.length === 0 && (
            ...
            >
              ...
              <span className="inline-block mt-4 px-3 py-1 bg-[rgba(0,0,0,0.02)] text-micro text-[var(--ink-muted)] rounded-full">
            ...
            >
              ...
              <div className="flex items-center justify-between text-xs bg-[#FFFBFC] p-4 rounded-2xl border border-[rgba(0,0,0,0.02)]">
                <div>
                  ...
                  <p className="text-[var(--ink-muted)] font-light">
                ...
                <div className="flex gap-1.5">
                  {files.some((f) => f.status === 'success') && (
                    <span className="px-2 py-0.5 bg-[#C5E8D8] text-[#348861] font-bold rounded-md text-micro">
                      สำเร็จ: {files.filter((f) => f.status === 'success').length}
                    </span>
                  )}
                  {files.some((f) => f.status === 'error') && (
                    <span className="px-2 py-0.5 bg-[#FFF0F3] text-[#F4A0B5] font-bold rounded-md text-micro">
                ...
                {files.map((file) => (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0 pr-6 relative">
                      ...
                      <p className="text-micro text-[var(--ink-muted)] font-light">{formatBytes(file.file.size)}</p>
                      ...
                      {file.status === 'success' && file.uploadedUrl && (
                        <div className="flex items-center gap-2 mt-1.5">
                          <button
                            ...
                            className={`px-2 py-0.5 rounded text-micro font-semibold transition-colors duration-150 flex items-center gap-1 cursor-pointer ${
                          ...
                          <a
                            ...
                            className="text-micro text-[var(--ink-muted)] hover:text-[#3D3040] underline font-light"
                          >
                            View Image
                          </a>
                        </div>
                      ...
                      {/* Error message */}
                      {file.status === 'error' && (
                        <p className="text-micro text-[#F4A0B5] font-medium mt-1 truncate" title={file.error}>
                    ...
                    {!isUploading && file.status !== 'success' && (
                      <button
                        ...
                        className="absolute top-2.5 right-2.5 w-6 h-6 rounded-full bg-neutral-50 hover:bg-neutral-100 text-[var(--ink-muted)] hover:text-[#3D3040] flex items-center justify-center transition-colors cursor-pointer border border-[rgba(0,0,0,0.02)]"
                        title="Remove file"
                      >
                        <svg
                          xmlns="http://www.w3.org/2000/svg"
              ...
              {hasUploadFinished && allSuccess && (
                ...
                >
                  ...
                  <p className="text-xs text-[var(--ink-muted)] font-light mb-3">รูปภาพของคุณอยู่ในระบบคลาวด์ R2 และฐานข้อมูลพร้อมใช้งานในแกลเลอรี</p>
                  <button
              ...
              <div className="flex gap-3 pt-2">
                <button
                  ...
                  className="flex-1 py-3 px-4 rounded-xl border border-[rgba(0,0,0,0.08)] text-sm font-medium text-[var(--ink-muted)] hover:text-[#3D3040] hover:bg-neutral-50 transition-all cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
                >
      ...
      {/* Footer signoff */}
      <div className="text-center text-micro text-[#C8BBC0] mt-12 font-light">
