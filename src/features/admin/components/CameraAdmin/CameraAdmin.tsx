    ...
    <section className="py-6 sm:py-8 px-3 sm:px-6 lg:px-8 w-full max-w-[1400px] mx-auto">
      ...
      {isLoading ? (
        <div className="py-20 flex flex-col items-center justify-center gap-4">
          ...
          <p className="text-xs text-[var(--ink-muted)]">กำลังโหลดรายการกล้องจาก Supabase...</p>
        </div>
      ) : errorMessage ? (
        ...
        <div className="py-16 text-center bg-white/60 rounded-3xl border border-[rgba(0,0,0,0.04)]">
          <p className="text-sm text-[var(--ink-muted)] mb-4">ยังไม่มีรุ่นกล้องในระบบ</p>
          <button
            onClick={openCreateModal}
          ...
          {cameras.map((camera) => (
            ...
            >
              ...
              <div className="relative w-full h-44 rounded-2xl overflow-hidden bg-neutral-100 mb-4 border border-[rgba(0,0,0,0.06)] shadow-2xs group-hover:scale-[1.01] transition-transform duration-300">
                ...
                <div className="absolute top-3 left-3">
                  <span
                    ...
                    className={`text-micro font-bold px-2.5 py-1 rounded-full backdrop-blur-md shadow-xs border cursor-pointer transition-all ${
                      camera.isActive
              ...
              <div className="space-y-2 flex-1">
                ...
                {camera.description && (
                  <p className="text-xs text-[var(--ink-muted)] font-light leading-relaxed whitespace-pre-line bg-[#FFFBFC] p-3 rounded-xl border border-[rgba(0,0,0,0.04)]">
                    {camera.description}
                  </p>
                )}
              </div>
