...
import {
  ...
  updateCameraAction,
  deleteCameraAction,
} from '@/features/admin/actions/camera-actions';
...
export function useCameraAdmin() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (editingCamera) {
      ...
      if (res.success) {
        ...
        await alertDialog({ title: res.message || 'เกิดข้อผิดพลาดในการบันทึกข้อมูล', danger: true });
      }
    } else {
      ...
      if (res.success) {
        ...
        await alertDialog({ title: res.message || 'เกิดข้อผิดพลาดในการสร้างกล้อง', danger: true });
      }
    }
    setIsSaving(false);
  };
  ...
  const handleDelete = async (id: number, name: string) => {
    if (!(await confirmDialog({ title: `คุณต้องการลบกล้องรุ่น "${name}" ใช่หรือไม่?`, confirmLabel: 'ลบรายการ', danger: true }))) return;
    setDeletingId(id);
    const res = await deleteCameraAction(id);
    if (res.success) {
      await loadCameras(true);
    } else {
      await alertDialog({ title: res.message || 'เกิดข้อผิดพลาดในการลบกล้อง', danger: true });
    }
    setDeletingId(null);
  };
