...
export default function ScheduleAdmin() {
  ...
  return (
    <section className="py-6 sm:py-8 px-3 sm:px-6 lg:px-8 w-full max-w-[1400px] mx-auto">
      {/* Header & Title */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 sm:mb-8">
        <div>
          <span className="inline-block text-xs sm:text-xs font-semibold tracking-[0.2em] uppercase text-[#F4A0B5] mb-1.5 sm:mb-2">
            ✦ Studio Schedule Management
          </span>
          <h2 className="font-display text-2xl sm:text-3xl lg:text-4xl font-bold text-[#3D3040]">
            จัดการคิวงาน & รอบเวลา
          </h2>
          <p className="text-xs sm:text-sm text-[var(--ink-muted)] font-light mt-1">
        ...
        <div className="flex items-center gap-2 sm:gap-3 flex-wrap sm:flex-nowrap">
          {/* View Mode Toggle */}
          <div className="bg-white/80 backdrop-blur-xl border border-[rgba(0,0,0,0.06)] rounded-2xl p-1 flex items-center shadow-xs flex-shrink-0">
            <button
              onClick={() => setViewMode('cards')}
              className={`px-3 sm:px-3.5 py-1.5 sm:py-2 rounded-xl text-xs font-semibold transition-all cursor-pointer flex items-center gap-1.5 ${
                viewMode === 'cards'
          ...
          <button
            ...
            className="flex-1 sm:flex-initial py-2 sm:py-2.5 px-3.5 sm:px-4 rounded-2xl bg-gradient-to-r from-[#F4A0B5] to-[#D4B5E0] hover:from-[#F4A0B5]/90 hover:to-[#D4B5E0]/90 text-white text-xs sm:text-sm font-semibold shadow-[0_4px_16px_rgba(244,160,181,0.25)] transition-all cursor-pointer flex items-center justify-center gap-2 hover:shadow-md hover:scale-[1.02]"
          >
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 sm:gap-4 mb-6 sm:mb-8 bg-white/80 backdrop-blur-xl p-3 sm:p-4 rounded-2xl sm:rounded-3xl border border-[rgba(0,0,0,0.04)] shadow-xs">
        <div className="relative w-full sm:w-80">
          <input
            ...
            className="w-full pl-9 pr-4 py-2 sm:py-2.5 rounded-xl sm:rounded-2xl border border-[rgba(0,0,0,0.07)] text-xs text-[#3D3040] bg-[#FFFBFC] focus:outline-none focus:ring-2 focus:ring-[#F4A0B5]/40"
          />
          <svg className="w-4 h-4 text-[var(--ink-muted)] absolute left-3 top-2.5 sm:top-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
          </svg>
        </div>

        <div className="flex items-center gap-2 w-full sm:w-auto justify-between sm:justify-end">
          <span className="text-xs font-medium text-[var(--ink-muted)] whitespace-nowrap">สถานะวัน:</span>
          <select
            value={statusFilter}
            onChange={e => setStatusFilter(e.target.value)}
            className="flex-1 sm:flex-initial px-3 sm:px-3.5 py-2 sm:py-2.5 rounded-xl sm:rounded-2xl border border-[rgba(0,0,0,0.07)] text-xs text-[#3D3040] bg-[#FFFBFC] focus:outline-none focus:ring-2 focus:ring-[#F4A0B5]/40 cursor-pointer font-medium"
          >
      ...
      {isLoading ? (
        <div className="py-20 flex flex-col items-center justify-center gap-4">
          ...
          <p className="text-xs text-[var(--ink-muted)]">กำลังโหลดตารางงานจาก Supabase...</p>
        </div>
      ) : errorMessage ? (
        ...
        <div className="py-16 text-center bg-white/60 rounded-3xl border border-[rgba(0,0,0,0.04)]">
          <p className="text-sm text-[var(--ink-muted)] mb-4">ไม่พบข้อมูลตารางงานตามเงื่อนไขที่ค้นหา</p>
          <button
            onClick={openCreateModal}
          ...
          {concertGroups.map((group) => {
            ...
            return (
              <div
                ...
                className="group relative bg-white/90 backdrop-blur-xl border border-[rgba(0,0,0,0.06)] rounded-[1.75rem] sm:rounded-[2rem] p-4 sm:p-5 shadow-[0_8px_30px_rgba(0,0,0,0.02)] transition-all duration-300 hover:shadow-[0_12px_40px_rgba(244,160,181,0.18)] hover:border-[#F4A0B5]/40 cursor-pointer flex flex-col justify-between overflow-hidden"
              >
                ...
                <div className="flex gap-3 sm:gap-4 items-start">
                  <div className="relative w-20 h-28 sm:w-24 sm:h-32 rounded-2xl overflow-hidden bg-neutral-100 flex-shrink-0 border border-[rgba(0,0,0,0.08)] shadow-2xs group-hover:scale-[1.02] transition-transform duration-300">
                    ...
                    <div className="absolute top-2 left-2">
                      <span className={`text-micro font-bold px-2 py-0.5 rounded-full backdrop-blur-md shadow-xs border ${statusClass}`}>
                        {statusLabel}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="flex-1 min-w-0 flex flex-col justify-between h-full">
                    <div>
                      <div className="inline-flex max-w-full items-center gap-1.5 px-2.5 py-1 rounded-xl bg-gradient-to-r from-[#FFFBFC] to-[#F9F5FA] border border-[rgba(0,0,0,0.06)] mb-2 shadow-2xs">
                        ...
                        <span className="font-mono text-micro sm:text-xs font-bold text-[#3D3040] truncate min-w-0" title={dateDisplay}>{dateDisplay}</span>
                      </div>
                      ...
                      <p className="text-xs text-[var(--ink-muted)] mt-1 flex items-center gap-1 truncate font-light">
                        <span>📍</span>
                    ...
                    {!isMultiDay && (
                      <div className="flex items-center justify-end gap-1.5 mt-3 pt-2 border-t border-[rgba(0,0,0,0.04)]">
                        <button
                          ...
                          className="p-1.5 rounded-xl border border-[rgba(0,0,0,0.08)] text-[var(--ink-muted)] hover:text-[#3D3040] hover:bg-neutral-50 transition-all cursor-pointer text-xs"
        ...
        <div className="bg-white rounded-[2rem] border border-[rgba(0,0,0,0.06)] shadow-lg overflow-hidden animate-fade-in">
          <div className="overflow-x-auto max-w-full">
            <table className="w-full text-left border-collapse min-w-[700px] sm:min-w-[900px]">
              <thead>
                <tr className="bg-[#FFFBFC] border-b border-[rgba(0,0,0,0.06)] text-xs font-bold text-[var(--ink-muted)]">
                ...
                {filteredSchedules.map((schedule, idx) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="p-4 text-[var(--ink-muted)]">
                        {schedule.location || '-'}
                      </td>

                      <td className="p-4 text-center">
                        <span className={`inline-block text-micro font-bold px-3 py-1 rounded-full border ${
                      ...
                      <td className="p-4 pr-6 text-right" onClick={(e) => e.stopPropagation()}>
                        <div className="flex items-center justify-end gap-2">
                          ...
                          <button
                            ...
                            className="p-1.5 rounded-xl border border-[rgba(0,0,0,0.08)] text-[var(--ink-muted)] hover:text-[#3D3040] hover:bg-neutral-50 transition-all cursor-pointer"
                            title="แก้ไขข้อมูล"
                          >
                            ✏️
                          </button>
      ...
      {activeSlotSchedule && (
        <SlotManager
          ...
          onEditSchedule={(sched) => openEditModal(sched)}
          handleToggleSlot={handleToggleSlot}
          setAllSlotsStatus={setAllSlotsStatus}
          onUpdateActiveSchedule={(updated) => setActiveSlotSchedule(updated)}
        />
