...
export default function BookingBotModal({
  ...
  const [date, setDate] = useState<string>(getTodayThailandDateString());
...
ชำระเงิน : ${paymentStatus === 'paid' ? `ชำระเงินเต็มจำนวนเรียบร้อยค่ะ 💚 (${depositAmount ? depositAmount.toLocaleString() : 0} ฿)` : paymentStatus === 'deposit' ? `ได้รับมัดจำเรียบร้อยค่ะ 🟡 (${depositAmount ? depositAmount.toLocaleString() : 0} ฿)${remainingAmount > 0 ? `\n💵 ยอดต้องเก็บเพิ่มอีก: ${remainingAmount.toLocaleString()} ฿` : ''}` : 'ยังไม่ได้ชำระ 🔴'}`;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-2 sm:p-4 animate-fade-in">
      <div className="bg-white rounded-2xl sm:rounded-3xl max-w-2xl w-full p-4 sm:p-7 shadow-2xl border border-[rgba(0,0,0,0.06)] relative max-h-[calc(100dvh-1.5rem)] sm:max-h-[92vh] flex flex-col">
        {/* Close Button */}
        <button
          type="button"
          onClick={onClose}
        ...
        {/* Modal Header */}
        <div className="flex items-center gap-2.5 sm:gap-3 mb-3 sm:mb-4 shrink-0 pr-8">
          <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-2xl bg-gradient-to-br from-[#F4A0B5]/20 to-[#D4B5E0]/30 border border-[#F4A0B5]/30 flex items-center justify-center text-lg sm:text-xl shrink-0">
            🤖
          </div>
          <div className="min-w-0">
            <h3 className="font-display text-base sm:text-xl font-bold text-[#3D3040] truncate">
              บอทพิมพ์แบบฟอร์มจองคิว
            </h3>
            <p className="text-xs sm:text-xs text-[var(--ink-muted)] font-light truncate">
              สร้างและคัดลอกแบบฟอร์มสำหรับ LINE Admin Bot และลูกค้า
            </p>
          </div>
        </div>

        {/* Navigation Tabs */}
        <div className="flex items-center gap-1.5 p-1 bg-neutral-100/80 rounded-2xl mb-3 sm:mb-5 shrink-0 overflow-x-auto no-scrollbar">
          <button
            onClick={() => setActiveTab('admin_bot')}
            className={`py-1.5 sm:py-2 px-3 sm:px-3.5 rounded-xl text-xs font-bold transition-all cursor-pointer flex items-center gap-1.5 whitespace-nowrap flex-shrink-0 ${
              activeTab === 'admin_bot'
        ...
        <div className="flex-1 min-h-0 overflow-y-auto pr-1 space-y-4">
          ...
          {activeTab === 'admin_bot' && (
            <div className="space-y-4 animate-fade-in">
              ...
              {/* Event Select */}
              <div>
                <label className="block text-xs font-bold text-[#3D3040] mb-1">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <select
                    value={paymentStatus}
                    onChange={e => setPaymentStatus(e.target.value as 'deposit' | 'paid' | 'unpaid')}
                    className="w-full px-3.5 py-2 rounded-xl border border-[rgba(0,0,0,0.1)] text-xs text-[#3D3040] focus:outline-none focus:ring-2 focus:ring-[#F4A0B5]/40 bg-white cursor-pointer"
                  >
              ...
              <div className="p-4 rounded-2xl bg-neutral-50 border border-[rgba(0,0,0,0.06)] space-y-3">
                ...
                <div className="space-y-2 font-mono text-xs">
                  <div className="p-2.5 rounded-xl bg-white border border-[rgba(0,0,0,0.06)]">
                    ...
                    <p className="text-micro text-[var(--ink-muted)] font-sans mt-0.5">ดึงตารางอีเวนต์ทั้งหมดในระบบและปุ่มเลือกเช็กคิวว่าง</p>
                  </div>
                  <div className="p-2.5 rounded-xl bg-white border border-[rgba(0,0,0,0.06)]">
                    ...
                    <p className="text-micro text-[var(--ink-muted)] font-sans mt-0.5">รายงานรายการคิวที่ถูกจองแล้วและรอบเวลาที่ยังว่างอยู่</p>
                  </div>
                  <div className="p-2.5 rounded-xl bg-white border border-[rgba(0,0,0,0.06)]">
                    <span className="font-bold text-[#F4A0B5]">3. แบบฟอร์ม</span> หรือ <span className="font-bold text-[#F4A0B5]">วิธีพิมพ์</span>
                    <p className="text-micro text-[var(--ink-muted)] font-sans mt-0.5">ให้บอทพ่นแบบฟอร์มเปล่ามาให้ก๊อปปี้ไปแก้ไขในแชท LINE</p>
                  </div>
                </div>
              </div>
              ...
              <div className="p-4 rounded-2xl bg-neutral-50 border border-[rgba(0,0,0,0.06)] space-y-3">
                ...
                <div className="space-y-2 font-mono text-xs">
                  <div className="p-2.5 rounded-xl bg-white border border-[rgba(0,0,0,0.06)]">
                    ...
                    <p className="text-micro text-[var(--ink-muted)] font-sans mt-0.5">แสดงการ์ดอีเวนต์ทั้งหมดที่เปิดรับคิว</p>
                  </div>
                  <div className="p-2.5 rounded-xl bg-white border border-[rgba(0,0,0,0.06)]">
                    <span className="font-bold text-[#F4A0B5]">2. แบบฟอร์ม</span> หรือ <span className="font-bold text-[#F4A0B5]">ขอแบบฟอร์ม</span>
                    <p className="text-micro text-[var(--ink-muted)] font-sans mt-0.5">ส่งแบบฟอร์มเปล่าให้ลูกค้าก๊อปไปกรอกข้อความ</p>
                  </div>
                  <div className="p-2.5 rounded-xl bg-white border border-[rgba(0,0,0,0.06)]">
                    <span className="font-bold text-[#F4A0B5]">3. พิมพ์: &quot;ชื่อ เบอร์โทร เวลาที่ต้องการ&quot;</span>
                    <p className="text-micro text-[var(--ink-muted)] font-sans mt-0.5">เช่น: คิมโดยอง 0812345678 12:00-12:20 (ระบบสร้างคิวลง Supabase อัตโนมัติ)</p>
                  </div>
                </div>
              </div>
            </div>
