...
export default function GalleryAdmin() {
  const [mobileLayout, setMobileLayout] = useState<'grid' | 'carousel'>('grid');
  const {
    photos,
    isLoading,
    currentPage,
  ...
  return (
    <section id="gallery" className="relative py-8 sm:py-16 px-4 sm:px-8 lg:px-12 max-w-6xl mx-auto">
      {/* Section Header */}
      <ScrollReveal className="text-center mb-8 sm:mb-10 relative">
          ...
          <AnimatePresence mode="wait">
            {saveStatus === 'saving' && (
              <motion.div
                ...
                exit={{ opacity: 0, y: 10 }}
                className="flex items-center gap-1.5 px-3 py-1 bg-white border border-[rgba(0,0,0,0.05)] shadow-sm rounded-full text-xs font-medium text-[var(--ink-muted)]"
              >
        ...
        <h2 className="font-display text-2xl sm:text-4xl lg:text-5xl font-bold text-[#3D3040] mb-3 sm:mb-5">
          Gallery Shots
        </h2>
        ...
        <p className="text-[var(--ink-muted)] max-w-lg mx-auto text-sm sm:text-base font-light leading-relaxed mb-6">
        ...
        <div className="flex justify-center mt-6">
          <Link
            href="/admin/upload"
            className="py-2.5 px-5 sm:py-3 sm:px-6 rounded-2xl bg-[#F4A0B5] hover:bg-[#F4A0B5]/90 text-white text-xs sm:text-sm font-semibold shadow-[0_4px_16px_rgba(244,160,181,0.2)] hover:shadow-[0_6px_20px_rgba(244,160,181,0.3)] transition-all flex items-center justify-center gap-2 cursor-pointer"
          >
      ...
      {totalPages > 1 && (
        <div className="mb-8 sm:mb-14">
            ...
            <button
              ...
              className={`w-10 h-10 sm:w-11 sm:h-11 rounded-full flex items-center justify-center text-sm transition-all duration-300 cursor-pointer ${
                currentPage === 1
                  ...
                  : 'text-[var(--ink-muted)] hover:text-[#3D3040] border border-[rgba(0,0,0,0.06)] hover:border-[#F4A0B5]/30 hover:bg-[#FFF5F7]'
              }`}
          ...
          {/* Page Info */}
          <p className="text-center text-[#C8BBC0] text-xs mt-3 sm:mt-4 tracking-wide">
      ...
      {isLoading ? (
        <div className="flex flex-col items-center justify-center py-20 sm:py-32 gap-4">
          ...
          <p className="text-[var(--ink-muted)] text-sm font-light">กำลังโหลดรูปภาพ...</p>
        </div>
      ) : (
        <>
          {/* Mobile View Toggle */}
          <div className="flex sm:hidden justify-center items-center gap-1.5 mb-5 bg-white/70 backdrop-blur-md p-1 rounded-xl border border-[rgba(0,0,0,0.06)] w-fit mx-auto shadow-xs">
            <button
              type="button"
              onClick={() => setMobileLayout('grid')}
              className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all cursor-pointer ${
                mobileLayout === 'grid'
                  ? 'bg-[#F4A0B5] text-white shadow-xs'
          ...
          {mobileLayout === 'carousel' ? (
            ...
            <div 
              ref={mobileScrollRef}
              className="flex sm:hidden overflow-x-auto snap-x snap-mandatory gap-4 pb-8 -mx-4 px-4 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]"
            >
              {paginatedPhotos.map((photo, index) => (
                <div key={`mobile-carousel-${photo.id}`} className="w-[85vw] shrink-0 snap-center">
                  <PhotoCard
                    photo={photo}
                    index={index}
                    priority={index < 2}
            ...
            <div className="grid grid-cols-2 gap-3 pb-8 sm:hidden">
              {paginatedPhotos.map((photo, index) => (
                <div key={`mobile-grid-${photo.id}`} className="relative">
                  <PhotoCard
                    photo={photo}
                    index={index}
                    priority={index < 2}
          ...
          <div className="hidden sm:grid sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 min-h-[600px] items-start">
            {paginatedPhotos.map((photo, index) => (
              ...
              >
                ...
                <div className="absolute top-3 left-3 z-10 w-8 h-8 rounded-full bg-white/90 text-[var(--ink-muted)] shadow-md flex items-center justify-center hover:text-[#3D3040] transition-colors pointer-events-none">
      ...
      {photoToDelete && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm transition-all duration-300">
          <div className="bg-white rounded-3xl p-6 sm:p-8 max-w-sm w-full border border-[rgba(0,0,0,0.05)] shadow-2xl transition-all duration-300 scale-100 text-[#3D3040] relative overflow-hidden">
            ...
            <p className="text-xs sm:text-sm text-[var(--ink-muted)] text-center font-light leading-relaxed mb-5">
            ...
            <div className="flex gap-3">
              <button
                ...
                disabled={isDeleting}
                className="flex-1 py-2.5 px-4 rounded-xl border border-[rgba(0,0,0,0.08)] text-xs sm:text-sm font-medium text-[var(--ink-muted)] hover:text-[#3D3040] hover:bg-neutral-50 transition-all cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
              >
                ยกเลิก
              </button>
              <button
